...
export class TransactionsService {
  ...
  async getUserList(userId: string): Promise<ITransactionUserDto[]> {
    return (await this._userModel.find({ _id: { $ne: userId } })).map(
      ({ _id, email, name }) => ({
        id: _id,
        email,
        name,
  ...
  async getTransactionsList(userId: string): Promise<ITransactionDto[]> {
    return (
      await this._transactionModel
        .find({
          $or: [
            { senderUser: { $eq: userId } },
            { recipientUser: { $eq: userId } },
          ],
        })
  ...
  async createTransaction(userId: string, dto: CreateTransactionDto) {
    ...
    if (!senderUser) {
      throw new HttpException(
        'The sender user not found',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (!recipientUser) {
      throw new HttpException(
        'The recipient user not found',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (senderUser.id === recipientUser.id) {
      throw new HttpException(
        "User can't s end pw to yourself",
        HttpStatus.BAD_REQUEST,
      );
    }
    if (amount > senderUser.balance) {
      throw new HttpException(
        'Amount is greater then balance',
        HttpStatus.BAD_REQUEST,
      );
    }
